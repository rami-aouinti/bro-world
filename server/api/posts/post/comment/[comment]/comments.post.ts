...
export default defineEventHandler(async (event) => {
  ...
  const comment = event.context.params?.comment

  if (!comment) {
    throw createError({ statusCode: 400, message: 'Missing comment' })
  }
  ...
  const url = `${config.public.apiBlogBase}/v1/platform/comment/${comment}/comment`
