...
<template>
  <v-card class="shadow-md">
    ...
    >
      ...
      <v-input v-model="search" placeholder="Filter" />
      <v-pagination v-model="page" :page-count="pageCount" :total="totalItems" />
      <v-select
        v-model="pageCount"
    ...
    >
      ...
      <template #loading-state>
        <div class="flex flex-col items-center justify-center h-48">
          ...
          <v-skeleton-loader v-for="i in 3" :key="i" class="h-1/2 w-1/3 m-2" />
          <v-skeleton-loader v-for="i in 3" :key="i" class="h-1/2 w-1/3 m-2" />
        </div>
      </template>
