...
export default defineEventHandler(async (event) => {
  ...
  try {
    const response = await axios.post('https://bro-world.org/api/v1/story', formData, {headers})
    ...
    if (err.response?.status === 401) {
      try {
        const retryResponse = await axios.post('https://bro-world.org/api/v1/story', formData, {headers})
