...
export default defineEventHandler(async (event) => {
  ...
  axiosFormData.append('mediaFolder', id)
  const config = useRuntimeConfig()
  const url = `${config.public.apiMediaBase}/v1/platform/mediaFolder`
  return await requestWithRetry('post', url, token, axiosFormData, false)
})
